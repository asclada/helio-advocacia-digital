...
- **Fase 5 — Banco do CRM no Supabase:** concluída, num repositório novo e separado ([`asclada/helio-advocacia-crm`](https://github.com/asclada/helio-advocacia-crm), privado) — o banco do CRM e, a partir da Fase 6, o painel autenticado vivem fora deste repo do site. Schema: `leads` (um registro único e "achatado" por lead triado pela IA) e `profiles` (ligada ao Supabase Auth), sem preparação multi-tenant, Row Level Security verificada pra acesso autenticado, anônimo e via service role.
- **Fase 6 — Painel autenticado do CRM:** concluída, no repo do CRM. Login dos 3 usuários reais do escritório via Supabase Auth, tabela de leads em `/clientes` (TanStack Table, ordenação/busca) e painel lateral de detalhe do lead. Estendida duas vezes: arquivamento de lead (6.1) e campo `resumo` com painel lateral de triagem completa substituindo a antiga página de detalhe (6.2).
- **Fase 7 — Migração do agente de IA:** concluída. O agente de triagem saiu do WhatsApp/Evolution API para um webhook HTTP síncrono consumido pelo proxy `/api/chat` deste repo, gravando no projeto Supabase correto do CRM (`leads` + nova tabela `triagens`). Saída estruturada em JSON agora popula todos os campos do lead, e a conclusão só acontece com pelo menos um contato (telefone ou e-mail). O antigo acompanhamento de inatividade ("Watchdog") foi abandonado sem substituto — não existe equivalente num modelo de request/response HTTP.
- **Fase 8 — Widget de chat:** concluída. O widget de chat flutuante (chip, balão de notificação, painel) vive neste repo e conversa com o webhook do n8n já migrado — conversas reais ponta a ponta verificadas em desktop e em celular real. Limitação conhecida: a conversa retoma com saudação pelo nome e escolha de continuar/recomeçar ao recarregar a página, exceto na janela estreita entre dar o nome e confirmar o aviso de LGPD, onde o agente ainda volta a pedir essa confirmação em vez de oferecer retomada — mesma categoria de não-determinismo de LLM aceito do Watchdog acima, documentada no system prompt do n8n.
- **Fases 9–10** (integração ponta a ponta, QA de produção): não iniciadas.
