...
- **Phases 4–10** (real page assembly, CRM database, CRM panel, migrating the AI agent to the website, the chat widget itself, end-to-end integration, production QA): not started.

The homepage currently renders a placeholder, not final content — real sections are assembled in Phase 4, once the base components exist. The nav's anchor links (`#atuacao`, `#sobre`, `#faq`, `#contato`) don't have a matching `<section>` yet for the same reason.
