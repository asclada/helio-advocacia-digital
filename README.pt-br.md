...
- **Fase 2 — Componentes de UI base:** em andamento. Concluído até agora: setup de testes (Vitest + Testing Library + jest-axe), wrapper de layout `Container`/`Section`, `Button` (variantes primary/secondary/ghost). Restam: `Badge`, `Card`, `Input` + `Label`.
- **Fases 3–10** (navegação/rodapé, montagem real das páginas, banco de dados do CRM, painel do CRM, migração do agente de IA para o site, o widget de chat em si, integração ponta a ponta, QA de produção): não iniciadas.

A home atualmente renderiza um placeholder, não conteúdo final — as seções reais são montadas na Fase 4, quando os componentes base já existirem.
