...
- **Phase 9 — End-to-end integration:** done. Split into two gated stages. Stage A re-validated the whole system against the deployed production site (not localhost): the 6 Phase 7 conversation scenarios re-run through the real widget, CRM cross-checks, and conversation resumption — 3 real bugs found and fixed (a missing production env var, a prompt-ambiguity bug that could skip the LGPD consent step under a specific opening-message pattern, and a chat-bubble line-wrap CSS bug on long unbroken tokens). Stage B added the one net-new feature identified while re-reading the phase's original goal: an e-mail notification to Dr. Hélio on every concluded lead, via n8n's native SMTP node, gated on the same Phase 7 guard (fires only when a lead actually gets a phone and/or e-mail on file).
- **Phase 10** (production deploy, final QA): not started.
