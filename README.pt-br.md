...
- **Fases 4–10** (montagem real das páginas, banco de dados do CRM, painel do CRM, migração do agente de IA para o site, o widget de chat em si, integração ponta a ponta, QA de produção): não iniciadas.

A home atualmente renderiza um placeholder, não conteúdo final — as seções reais são montadas na Fase 4, quando os componentes base já existirem. Os links de âncora do nav (`#atuacao`, `#sobre`, `#faq`, `#contato`) ainda não têm uma `<section>` real correspondente pelo mesmo motivo.
